...
void _cpm_srcs_create(BuildProperties_t *buildprop, ...)
{
  ...
  StringBuilder_t *builder = sb_new();
  while (arg != NULL)
  {
...
void _cpm_cflags_create(BuildProperties_t *buildprop, ...)
{
  va_list args;
  va_start(args, buildprop);
  StringBuilder_t *resbuild = sb_new();
  char *arg = va_arg(args, char *);
  while (arg != NULL)
  {
...
char *cpm_glob_dir(char *dirpath, const char *pattern)
{
  CPMLOG(CPM_ERROR, "TODO: glob implementation for windows not available yet! sowwyy... >w<")
}
#else
char *cpm_glob_dir(char *dirpath, const char *pattern)
{
